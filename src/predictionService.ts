...
export interface PredictionRequestData {
  features: Record<string, any>; // Input features for the model
  modelId?: string; // Optional: specify a model to use
  context?: Record<string, any>; // Optional: additional context for prediction
}
...
export interface PredictionServiceResponse extends PredictionUpdate {
  // any additional fields specific to this service's response
}
...
class PredictionService {
    ...
    async getPredictionDetails(predictionId: string): Promise<any> {
 ...
 */
export const getPredictionDetails = async (predictionId: string): Promise<any> => {
