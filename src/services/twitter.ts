...
class TwitterService {
  ...
  async searchTweets(query: string, options?: {
    ...
    try {
      ...
      const response = await apiService.get<Tweet[]>(
        '/twitter/search',
        params
      );
      return response;
  ...
  async getSentimentAnalysis(entity: string, options?: {
    startTime?: string;
    endTime?: string;
    aspects?: string[];
    ...
    try {
      ...
      const response = await apiService.get<SentimentAnalysis>(
        '/twitter/sentiment',
        params
      );
      return response;
  ...
  async getUserSentiment(username: string): Promise<{
    ...
    try {
      const response = await apiService.get(
        `/twitter/users/${username}/sentiment`,
        { apiKey: this.config.apiKey }
      );
      return response;
    } catch (error) {
