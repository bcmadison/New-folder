...
class SportsBettingService {
  ...
  private determineOptimalBet(
    ...
    const ev = {
      ...
      draw: odds[0].drawOdds ? probabilities.draw * odds[0].drawOdds - (1 - probabilities.draw) : -1
    };
    ...
    const kellyFraction = this.calculateKellyStake(
      bestBet.type === 'home' ? odds[0].homeOdds : 
      bestBet.type === 'away' ? odds[0].awayOdds :
      odds[0].drawOdds || 0,
      bestBet.type === 'home' ? probabilities.home :
      bestBet.type === 'away' ? probabilities.away :
      probabilities.draw
    );
    ...
    return {
      ...
      stake: adjustedStake,
      odds: bestBet.type === 'home' ? odds[0].homeOdds :
            bestBet.type === 'away' ? odds[0].awayOdds :
            odds[0].drawOdds || 0,
