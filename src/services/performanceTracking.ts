...
class PerformanceTrackingService {
  ...
  constructor() {
    console.log('[PerformanceTrackingService] Initialized');
  ...
  public startTrace(name: string, op: string, description?: string): Transaction | undefined {
    ...
    console.log(`[PerformanceTrackingService] Trace started: ${name} (Op: ${op})`);
    return transaction;
  }
  ...
  public addSpanToTrace(parentTransaction: Transaction | undefined, op: string, description?: string, data?: Record<string, any>): Span | undefined {
    ...
    // console.log(`[PerformanceTrackingService] Span added: ${op} to trace ${parentTransaction.name}`);
    return span;
  }
  ...
  public endTrace(transaction: Transaction | undefined): void {
    if (transaction) {
      ...
      // console.log(`[PerformanceTrackingService] Trace finished: ${transaction.name}`);
    } else {
  ...
  public endSpan(span: Span | undefined): void {
    if (span) {
      span.finish();
      // console.log(`[PerformanceTrackingService] Span finished: ${span.op}`);
    }
  }
  ...
  public recordMetric({
    ...
    console.log(`[PerformanceTrackingService] Recording metric: ${name} = ${value} (${type})`);
    switch (type) {
      case 'distribution':
  ...
  public setTag(key: string, value: string | number | boolean): void {
    ...
    // console.log(`[PerformanceTrackingService] Tag set: ${key} = ${value}`);
  }
  ...
  public setExtra(key: string, data: any): void {
    ...
    // console.log(`[PerformanceTrackingService] Extra context set: ${key}`);
  }
}
