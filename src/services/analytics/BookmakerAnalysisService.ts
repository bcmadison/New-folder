...
class BookmakerAnalysisService {
  ...
  private async initializeService() {
    try {
      ...
      this.patterns = new Map(historicalData.map(pattern => [
        `${pattern.tag}`,
        pattern
      ]));
  ...
  private async updatePatternAnalysis() {
    ...
    for (const tagType of ['demon', 'goblin', 'normal'] as const) {
      ...
      const deviations = tagsOfType.map(tag => 
        Math.abs(tag.projectedValue - tag.actualValue)
      );
  ...
  private async savePatterns() {
    try {
      await apiClient.post('/analytics/bookmaker/patterns', 
  ...
  public async analyzeProp(
    ...
    if (this.isSuspiciouslyFavorable(rawStatisticalProbability, bookmakerIntent)) {
      warnings.push(
        'This prop appears suspiciously favorable. Exercise caution.'
      );
    }

    if (pattern && pattern.successRate < 0.4) {
      warnings.push(
        `Historical success rate for ${propData.tag} props is unusually low (${
  ...
  private async analyzeBookmakerIntent(
    ...
    const suspiciousLevel = this.calculateSuspiciousLevel(
      propData,
      pattern
    );

    const historicalAccuracy = pattern 
      ? pattern.successRate 
      : 0.5;
  ...
  private async analyzeMarketTrend(
    propData: {
      tag: BookmakerTag['type'];
      currentOdds: number;
    }
    ...
    try {
      const response = await apiClient.get(
        `/analytics/market-trend/${propData.tag}`
      );
