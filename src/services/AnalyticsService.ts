...
import { BetRecord, BettingOpportunity, RiskAssessment } from '../types/core';
...
export class AnalyticsService {
  ...
  private setupEventListeners(): void {
    ...
    this.eventBus.on('prediction:update', (event) => {
    ...
    this.eventBus.on('data:updated', (event) => {
      if (event.sourceId === 'risk-manager') {
        const assessment = event.data[0] as RiskAssessment;
  ...
  public getPlayerBreakdown(): PlayerBreakdown[] {
    ...
    for (const bet of this.bets.values()) {
      ...
      stats.averageOdds += bet.odds;
      if (bet.result === 'win') stats.winRate++;
      if (!stats.metrics.includes(bet.metric)) {
  ...
  public getTimeSeriesData(interval: string = '1d'): TimeSeriesData[] {
    const now = Date.now();
    ...
    return this.timeSeriesData.filter(point => 
      now - point.timestamp <= intervalMs
    );
  }
