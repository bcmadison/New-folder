...
export const initialPrizePicksState: Pick<PrizePicksSlice, 'props' | 'currentPrizePicksPlayer' | 'currentPrizePicksLines' | 'entries' | 'isLoadingProps' | 'isLoadingEntries' | 'isLoadingPlayer' | 'isLoadingLines' | 'error'> = {
  props: [],
...
export const createPrizePicksSlice: StateCreator<
  AppStore,
  [],
  [],
  PrizePicksSlice
  ...
  fetchPrizePicksPlayer: async (playerIdOrName) => {
    ...
    try {
      ...
      const errorMsg = e.message || 'Failed to fetch player';
       ...
       get().addToast({message: `Error fetching player ${playerIdOrName}: ${errorMsg}`, type: 'error'});
    }
  },
