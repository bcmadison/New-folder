...
export const initialBetSlipState: Pick<BetSlipSlice, 'legs' | 'stake' | 'potentialPayout' | 'isSubmitting' | 'error'> = {
  legs: [],
  stake: 0,
  potentialPayout: 0,
  isSubmitting: false,
...
> = (set, get) => ({
  ...
  addLeg: (leg) => {
    ...
    if (!leg.odds) {
        addToast({ message: 'Cannot add leg without odds. Please select a valid prop line.', type: 'error'});
    ...
    if (legs.length >= 6) { // Max 6 legs for PrizePicks style parlays, adjust if needed
  ...
  calculatePotentialPayout: () => {
    ...
    else if (legs.length === 5) multiplier = 20; // Example, adjust based on actual PrizePicks rules
    else if (legs.length === 6) multiplier = 25; // Example
    else {
  ...
  submitSlip: async () => {
    ...
    if (legs.length < 2 || legs.length > 6) { // Typical PrizePicks rules
    ...
    try {
      ...
      if (confirmation && confirmation.length > 0 && confirmation[0].success) {
        addToast({ message: `Bet submitted successfully! ID: ${confirmation[0].betId.substring(0,8)}`, type: 'success' });
      } else {
