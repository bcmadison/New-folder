...
class PluginSystemSingleton {
  ...
  constructor() {
    ...
    this.pluginContext = {
      log: (message: string, level: 'info' | 'warn' | 'error' = 'info') => {
        ...
        console.log(`[PluginContext][${level.toUpperCase()}] ${message}`);
      },
  ...
  public async registerPlugin(plugin: Plugin): Promise<void> {
    ...
    console.log(`[PluginSystem] Registering plugin: ${plugin.name} v${plugin.version}`);
    ...
    try {
      ...
      console.log(`[PluginSystem] Initializing plugin: ${plugin.name} v${plugin.version}`);
      ...
      console.log(`[PluginSystem] Plugin "${plugin.name}" initialized successfully.`);
    } catch (error) {
  ...
  public async unloadPlugin(name: string): Promise<void> {
    ...
    if (plugin) {
      console.log(`[PluginSystem] Unloading plugin: ${name}`);
      if (plugin.destroy) {
        try {
          await plugin.destroy();
          console.log(`[PluginSystem] Plugin "${name}" destroyed successfully.`);
        } catch (error) {
  ...
  public async unloadAllPlugins(): Promise<void> {
    console.log('[PluginSystem] Unloading all plugins...');
...
//     // console.log('Sample Data Source URL:', config);
...
//   },
//   async destroy() {
//     console.log('SampleDataConnector Destroying...');
//   },
