...
class UnifiedPredictionEngineSingleton {
  ...
  constructor() {
    console.log('[UnifiedPredictionEngine] Initialized - Client-side interface to backend ML services');
  ...
  public async getPrediction(input: PredictionInput, modelId?: string): Promise<PredictionOutput | null> {
    ...
    try {
      console.log(`[UnifiedPredictionEngine] Requesting prediction for prop: ${input.propId}`, { input, modelId });
  ...
  public async getPredictionDetails(predictionId: string): Promise<any | null> {
    ...
    try {
      console.log(`[UnifiedPredictionEngine] Fetching details for prediction: ${predictionId}`);
...
//   if (prediction) {
//     console.log('Prediction received:', prediction.predictedOutcome, prediction.confidence);
//   }
// }); 