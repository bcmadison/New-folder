// src/core/UnifiedDataEngine.ts

import { prizePicksService } from '../services/prizePicksService';
import { dataScrapingService } from '../services/dataScrapingService';
// Import other relevant services as they are created (e.g., for Sportradar, TheOdds)
import { newsService } from '../services/newsService'; // Added for consistency with ComprehensivePlayerProfile
import { sentimentService } from '../services/sentimentService'; // Corrected import
import { poeToApiAdapter } from '../adapters/poe/PoeToApiAdapter'; // Added Poe Adapter

import {
  PrizePicksProps, DailyFantasyProjection, SocialSentimentData, OddsData, ESPNHeadline,
  // Add other relevant data types from ../types
} from '../types';
...
export class UnifiedDataEngineSingleton {
  ...
  constructor() {
    console.log('[UnifiedDataEngine] Initialized - Client-side orchestrator');
  ...
  public async getComprehensivePlayerProfile(playerNameOrId: string, leagueFilter?: string): Promise<ComprehensivePlayerProfile | null> {
    ...
    try {
      console.log(`[UnifiedDataEngine] Fetching comprehensive profile for: ${playerNameOrId}`);
  ...
  public async getMarketDetails(propId: string): Promise<any | null> {
    ...
    try {
        console.log(`[UnifiedDataEngine] Fetching market details for prop: ${propId}`);
  ...
  public async fetchPropsFromPoeSource(): Promise<PrizePicksProps[]> {
    ...
    try {
      console.log('[UnifiedDataEngine] Fetching props from Poe source via adapter...');
      const props = await poeToApiAdapter.fetchAndTransformPoeData();
      unifiedMonitor.endTrace(trace);
      return props;
    } catch (error: any) {
