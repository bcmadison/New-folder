...
// Clear all mocks before each test (if not using clearMocks: true in jest.config.js)
// beforeEach(() => {
//   jest.clearAllMocks();
// });
...
jest.mock('../core/UnifiedConfig', () => {
  ...
  const config = {
    ...
    experiments: [],
    apiEndpoints,
    getApiEndpoint: (key: string) => (typeof key === 'string' ? (apiEndpoints[key] !== undefined ? apiEndpoints[key] : `/api/${key}`) : ''),
