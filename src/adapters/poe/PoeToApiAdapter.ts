...
import { PrizePicksProps } from '../../types';
...
export class PoeToApiAdapter {
  constructor() {
    console.log('[PoeToApiAdapter] Initialized');
  }
      ...
      for (const block of poeDataBlocks) {
        if (block.type === 'prop_card' && block.content) {
          ...
          const prop: PrizePicksProps = {
            ...
            league: content.statType.includes('NBA') ? 'NBA' : content.statType.includes('NFL') ? 'NFL' : 'Unknown', // Crude league detection
