...
import { UnifiedDataEngine, DataStream } from './UnifiedDataEngine';
import { UnifiedMonitor } from './UnifiedMonitor';
import { UnifiedPredictionEngine, PredictionFactor } from './UnifiedPredictionEngine';
import { UnifiedPredictionService, PredictionResult } from '../services/unified/UnifiedPredictionService';
...
export class UnifiedStrategyEngine {
  ...
  private readonly strategies: Map<string, (context: StrategyContext) => Promise<StrategyRecommendation>>;
  ...
  private async handleMarketUpdate(update: MarketUpdate): Promise<void> {
    ...
    try {
      ...
      this.performanceMonitor.endTrace(traceId, error as Error);
      this.monitor.logError('strategy', error as Error);
    }
  }
  ...
  private async generateRecommendation(context: StrategyContext): Promise<StrategyRecommendation> {
    ...
    try {
      ...
      const validRecommendations = recommendations.filter(r => r !== null) as StrategyRecommendation[];
  ...
  private async getPredictionState(playerId: string, metric: string): Promise<StrategyContext['predictionState']> {
  ...
  private updateMetrics(strategyId: string, recommendation: StrategyRecommendation): void {
    ...
    this.metrics.set(strategyId, {
      ...
      averageConfidence: (currentMetrics.averageConfidence * currentMetrics.totalRecommendations + recommendation.confidence) / (currentMetrics.totalRecommendations + 1),
      lastUpdate: Date.now()
    });
  }

  private combineRecommendations(recommendations: StrategyRecommendation[]): StrategyRecommendation {
    ...
    const weightedRecommendations = recommendations.map(rec => {
      ...
      const successRate = metrics ? metrics.successfulRecommendations / metrics.totalRecommendations : 0.5;
      return {
        ...rec,
    ...
    return {
      ...
      confidence: weightedRecommendations.reduce((sum, rec) => sum + rec.confidence * rec.weight, 0) / totalWeight,
      expectedValue: weightedRecommendations.reduce((sum, rec) => sum + rec.expectedValue * rec.weight, 0) / totalWeight,
  ...
  private getMostCommonType(recommendations: Array<StrategyRecommendation & { weight: number }>): 'OVER' | 'UNDER' {
  ...
  private async analyzeParlay(props: DataPoint[]): Promise<BettingOpportunity | null> {
    ...
    const combinedConfidence = predictions.reduce(
    ...
    const combinedOdds = props.reduce(
      (acc, prop) => acc * prop.value.odds,
      1
    );
  ...
  public assessRisk(currentBets: BettingOpportunity[]): RiskAssessment {
    const currentExposure = currentBets.reduce(
      (total, bet) => total + bet.recommendedStake,
      0
    );

    const maxAllowedStake = Math.max(
      0,
      this.strategyConfig.maxExposure - currentExposure
    );
  ...
  private identifyRiskFactors(bets: BettingOpportunity[], exposure: number): string[] {
    ...
    for (const bet of bets) {
      for (const prop of bet.props) {
        ...
        gameConcentration.set(
          gameId,
          (gameConcentration.get(gameId) || 0) + bet.recommendedStake
        );
      }
    }
