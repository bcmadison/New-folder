...
export class UnifiedPredictionEngine {
  ...
  public async generatePrediction(context: PredictionContext): Promise<BettingOpportunity> {
    ...
    try {
      ...
      const modelPredictions = await this.getModelPredictions(context);
      
      // Filter out null predictions if any model failed
      const validModelPredictions = modelPredictions.filter(p => p.prediction !== null) as Array<{ id: string; prediction: ModelPrediction; weight: number }>;

      if (validModelPredictions.length === 0) {
        // Handle case where no models could generate a prediction
        // This might involve logging an error, returning a default opportunity, or throwing an exception
        this.monitor.logError('prediction', new Error('No valid model predictions generated.'));
        // Depending on desired behavior, you might return a default/error opportunity or throw
        // For now, let's throw an error to indicate failure.
      ...
      const analysis = await this.generateAnalysis(context, validModelPredictions);
  ...
  private async generateModelPrediction(
    ...
    try {
      ...
      this.monitor.logError('prediction', error as Error);
      return null;
    }
  }
  ...
  private combineModelPredictions(
    ...
    const weightedValue = predictions.reduce(
      (sum, p) => sum + p.prediction.value * p.weight,
      0
    ) / totalWeight;
    ...
    predictions.forEach(p => 
      p.prediction.analysis.risk_factors.forEach(rf => riskFactors.add(rf))
    );
  ...
  private async generateAnalysis(
    ...
    const marketSignals = context.marketState 
      ? this.analyzeMarketSignals(context.marketState)
      : [];
    ...
    return {
      ...
      risk_factors: Array.from(new Set(predictions.flatMap(p => p.prediction.analysis.risk_factors))),
      data: {
        historicalTrends,
        marketSignals
      }
  ...
  private setupEventListeners(): void {
    this.eventBus.on('market:update', async (update: MarketUpdate) => {
      try {
  ...
  private async generateTimeSeriesPrediction(context: PredictionContext): Promise<ModelPrediction | null> {
    try {
      ...
      if (!timeSeriesData || timeSeriesData.length === 0) {
        return {
          value: 0,
          confidence: 0.1,
          factors: [{ name: 'no_historical_data', weight: 1, source: 'time_series', confidence: 0.1 }],
          analysis: {
            risk_factors: ['insufficient_data_for_time_series'],
            meta_analysis: { market_efficiency: 0.5, playerId: context.playerId, metric: context.metric },
          },
        };
      }
      ...
      const predictedValue = context.marketState ? context.marketState.line * (1 + trendValue * 0.05) : 50 + trendValue * 5;

      return {
        value: predictedValue,
        confidence: 0.6,
        factors: [
          { name: 'historical_trend_simplified', weight: 1, source: 'time_series_analytics_service', confidence: 0.6 }
        ],
        analysis: {
      ...
      this.monitor.logError('generateTimeSeriesPrediction', error as Error);
      return null;
    }
  }

  private async generateMarketAnalysisPrediction(context: PredictionContext): Promise<ModelPrediction | null> {
    try {
      ...
      if (!playerAnalysis || !playerAnalysis.meta_analysis) {
         return {
          ...
          confidence: 0.2,
          factors: [{ name: 'no_advanced_market_analysis', weight: 1, source: 'market_analysis', confidence: 0.2 }],
          analysis: {
      ...
      return {
        ...
        factors: [
          { name: 'market_efficiency_factor', weight: 0.7, source: 'advanced_market_analysis', confidence: marketEfficiency },
          { name: 'prediction_stability_factor', weight: 0.3, source: 'advanced_market_analysis', confidence: playerAnalysis.meta_analysis.prediction_stability || 0.5 }
        ],
        analysis: {
          risk_factors: ['market_volatility_proxy', ...(playerAnalysis.risks && Object.keys(playerAnalysis.risks).length > 0 ? Object.values(playerAnalysis.risks).flatMap(r => r.factors) : ['unknown_market_risks'])],
          meta_analysis: {
      ...
      this.monitor.logError('generateMarketAnalysisPrediction', error as Error);
      return null;
    }
  }

  private async generatePerformanceAnalysisPrediction(context: PredictionContext): Promise<ModelPrediction | null> {
    try {
      const playerAnalysis = await this.advancedAnalysisEngine.analyzePlayer(context.playerId);
      ...
      if (!playerAnalysis || !playerAnalysis.predictions[context.metric]) {
        return {
          value: 0,
          confidence: 0.2,
          factors: [{ name: 'no_player_specific_prediction', weight: 1, source: 'performance_analysis', confidence: 0.2 }],
          analysis: {
      ...
      return {
        ...
        analysis: {
          risk_factors: playerAnalysis.risks && Object.keys(playerAnalysis.risks).length > 0 ? Object.values(playerAnalysis.risks).flatMap(r => r.factors) : ['general_performance_variance'],
          meta_analysis: {
      ...
      this.monitor.logError('generatePerformanceAnalysisPrediction', error as Error);
      return null;
    }
  }
} 