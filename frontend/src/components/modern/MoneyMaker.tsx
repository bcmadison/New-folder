...
import { BettingDecision, Projection } from '../../types/core';
...
const MoneyMaker: React.FC = () => {
    ...
    try {
      ...
      for (const [sport, sportProjections] of Object.entries(sportGroups)) {
        ...
        const decisions = await Promise.all(
          sportProjections.map(proj => 
            bettingSystem.analyzeBettingOpportunity({
              playerId: proj.playerId,
              metric: proj.propType,
              timestamp: Date.now(),
              marketState: proj.status,
              correlationFactors: []
            })
          )
        );
  ...
  const generateParlaysFromDecisions = (
    ...
    for (let size = 2; size <= maxParlaySize; size++) {
      ...
      for (const combo of combinations) {
        const projectionSet = combo.map(decision => 
          projections.find(p => p.playerId === decision.propId)!
        );

        // Calculate combined metrics
        const combinedConfidence = combo.reduce((acc, d) => acc * d.confidence, 1);
        const combinedEV = combo.reduce((acc, d) => acc + d.expectedValue, 0) / combo.length;
        ...
        const historicalTrends = Array.from(new Set(
          combo.flatMap(d => d.analysis.historicalTrends)
        ));
        const marketSignals = Array.from(new Set(
          combo.flatMap(d => d.analysis.marketSignals)
        ));
        const riskFactors = Array.from(new Set(
          combo.flatMap(d => d.analysis.riskFactors)
        ));
  ...
  const placeBet = (parlay: ParlayCard) => {
    ...
    const bet = {
      ...
      type: 'PARLAY',
      line: 0, // Not applicable for parlays
