...
const getSentimentBadge = (sentiment?: { score: number; direction: 'up' | 'down' | 'neutral'; tooltip?: string }) => {
  if (!sentiment) return null;
  const color = sentiment.direction === 'up' ? 'bg-green-100 text-green-700' : sentiment.direction === 'down' ? 'bg-red-100 text-red-700' : 'bg-gray-200 text-gray-700';
  const icon = sentiment.direction === 'up' ? '▲' : sentiment.direction === 'down' ? '▼' : '−';
  return (
    <span className={`ml-2 px-2 py-1 rounded-full text-xs ${color} cursor-help`} title={sentiment.tooltip || ''}>
  ...
  return (
    <main className="section space-y-6 lg:space-y-8 animate-fade-in">
      <div className="modern-card p-6 lg:p-8">
            ...
            >
              <div className="p-4 lg:p-6">
                ...
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <span className="text-sm font-bold">{(prop.winningProp.percentage * 100).toFixed(1)}%</span>
                  <span className="text-xs text-orange-500 ml-2">🔥{prop.pick_count}</span>
                </div>
                {(prop as any)?.espnNews && (
                  <a
                    href={typeof (prop as any).espnNews === 'string' ? undefined : (prop as any).espnNews?.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-blue-600 hover:underline"
                  >
                    📰 {typeof (prop as any).espnNews === 'string' ? (prop as any).espnNews : (prop as any).espnNews?.title}
                  </a>
                )}
              </div>
            </div>
        ...
        {/* Detail Modal (stub) */}
        {selected && (
          <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-6 backdrop-blur-sm" onClick={() => setSelected(null)}>
            <div className="modern-card max-w-2xl w-full p-8 text-gray-900 dark:text-white max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <h3 className="text-xl font-bold mb-4">Prop Details</h3>
              <pre className="text-xs whitespace-pre-wrap break-all">{JSON.stringify(props.find(p => p.player_name + p.stat_type + p.game_time === selected), null, 2)}</pre>
              <button className="modern-button mt-6" onClick={() => setSelected(null)}>Close</button>
            </div>
          </div>
        )}
      </div>
