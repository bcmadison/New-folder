...
export const PerformanceModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => (
  open ? (
...
export const LineupComparisonModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => (
  open ? (
...
export const EntryProgressModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => (
  open ? (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-6 backdrop-blur-sm">
...
export const PropDetailModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => (
  open ? (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-6 backdrop-blur-sm">
      <div className="modern-card max-w-2xl w-full p-8 text-gray-900 dark:text-white max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-start mb-8">
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-2xl font-light">←</button>
        </div>
