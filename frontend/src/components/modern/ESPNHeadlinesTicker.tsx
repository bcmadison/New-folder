...
const getSentimentBadge = (sentiment: PlayerSentiment) => {
  ...
  const avg =
    ...
    [sentiment.twitter, sentiment.reddit, sentiment.news]
      ...
      .reduce((a, b) => a + (b || 0), 0) /
    ([sentiment.twitter, sentiment.reddit, sentiment.news].filter((v) => typeof v === 'number').length || 1);
  ...
  return (
    <span className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${color}`}>{icon} {(avg * 100).toFixed(0)}%</span>
  );
};
...
const ESPNHeadlinesTicker: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let clientWidth = ticker.clientWidth;
  ...
  if (!headlines.length) {
    return (
      <div className="glass rounded-2xl px-4 py-2 text-gray-500 animate-pulse-soft">Loading ESPN headlines...</div>
    );
  }
