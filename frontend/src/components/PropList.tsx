...
export const PropList: React.FC<PropListProps> = ({ onPropSelect }) => {
  ...
  const [filter, setFilter] = useState<'all' | 'goblins' | 'demons' | 'value-bets' | 'high-confidence'>('all');
    ...
    <div className="space-y-4">
      ...
      <div className="flex space-x-2 mb-4 overflow-x-auto pb-2">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full transition-colors ${
            filter === 'high-confidence' ? 'bg-purple-500 text-white' : 'bg-gray-200 hover:bg-gray-300'
          }`}
        >
          High Confidence
        </button>
