...
export class BettingStrategyService {
  ...
  private validateMarket(market: string, books: BookOdds[]): void {
    ...
    const invalidOdds = books.find(book => 
      !book.odds || book.odds <= 0 || !Number.isFinite(book.odds)
    );
    if (invalidOdds) {
  ...
  public detectArbitrage(markets: Map<string, BookOdds[]>): BettingOpportunity[] {
    ...
    try {
      for (const [market, books] of markets) {
        this.validateMarket(market, books);
        ...
        if (totalImpliedProb < 1) {
          ...
          opportunities.push({
            id: `arb_${market}_${Date.now()}`,
            sport: 'UNKNOWN',
            gameId: 'UNKNOWN',
            market,
            type: 'ARBITRAGE',
            description: `Arbitrage opportunity in ${market}`,
            books: bestOdds,
            expectedValue,
      ...
      // Don't throw, return empty array to allow system to continue
      return [];
    }
    ...
    try {
      for (const [market, books] of markets) {
        ...
        for (let i = 0; i < sortedSpreads.length - 1; i++) {
          ...
          if (middleSize >= 1) { // Minimum 1 point middle
            ...
            if (ev > 0) {
              opportunities.push({
                id: `middle_${market}_${Date.now()}`,
                sport: 'UNKNOWN',
                gameId: 'UNKNOWN',
                market,
                type: 'MIDDLE',
                description: `Middle opportunity in ${market}`,
  ...
  private findBestOdds(books: BookOdds[]): BookOdds[] {
    return books.reduce((acc, book) => {
      const existingBook = acc.find(b => 
        Math.abs(this.oddsToSpread(b.odds) - this.oddsToSpread(book.odds)) < 0.5
      );
  ...
  private calculateTotalTeaserOdds(legs: TeaserLeg[]): number {
    return legs.reduce((total, leg) => total * this.americanToDecimal(leg.adjustedOdds ?? leg.originalOdds ?? leg.odds ?? 0), 1);
  }

  private calculateTeaserEV(legs: TeaserLeg[], totalOdds: number): number {
    const winProb = legs.reduce((prob, leg) => {
      const individualProb = 1 / this.americanToDecimal(leg.adjustedOdds ?? leg.originalOdds ?? leg.odds ?? 0);
  ...
  private createOpportunity(
    ...
    description: string = `${type} opportunity in ${market}`
  ): BettingOpportunity {
    return {
      id: uuidv4(),
      sport,
