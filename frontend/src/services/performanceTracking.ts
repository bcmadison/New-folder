...
export class PerformanceTrackingService {
  ...
  private calculateOptimalKellyMultiplier(bets: BetResult[]): number {
    ...
    const avgWinOdds = bets
      .filter(bet => bet.result === 'WIN')
      .reduce((sum, bet) => sum + bet.placedOdds, 0) / (bets.filter(bet => bet.result === 'WIN').length || 1);
  ...
  static analyzeClv(bet: BetRecord): ClvAnalysis {
    ...
    const timeToClose = bet.metadata.closingLine ? 
      (bet.metadata.closingLine - bet.timestamp) / 3600000 : 0; // Convert to hours
  ...
  private static calculateMarketEfficiency(bet: BetRecord): number {
    ...
    const timeToClose = bet.metadata.closingLine ? 
      (bet.metadata.closingLine - bet.timestamp) / 3600000 : 0;
